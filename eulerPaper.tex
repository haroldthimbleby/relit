...
Of course it is unlikely that a paper would present the above code explicitly: in the present paper, this \texttt{\%{}generate} command could have appeared after \LaTeX's \verb|\end{document}| so it would disappear and not be typeset as part of the published paper, as can any ``hidden'' definitions for any details that need not be part of a typical published paper. As we will discuss in section~\ref{invisible-code} below \name{relit} can show that, even taking account of the full listing above, a declaration is still hidden from the reader.

There is no restriction on the order of generating files: they can be specified in any order that best suits the needs of the author. However, because the code was written above, as shown, it actually works \emph{exactly\/} as written: it generates the file \texttt{euler.c} which was used to check the code presented in this paper. 

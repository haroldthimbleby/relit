...
\acmformat{[Submitted] H. Thimbleby, and D. Williams, How to publish algorithms reproducibly 
and
a new elegant 
algorithm 
% to generate $(N,2)$\\de Bruijn sequences with application to\\
...
\newcount\linesofcode
Our \emph{complete\/} working program, as will be fully described in this paper (shown in full section~\ref{complete-program}), is just \myinput{linesofcode.tex}
\newcount\denom \denom=\linesofcode
\ifnum \denom=0 \denom=1 % in case the file doesn't exist
\fi
\newcount\testcode 
...
\advance \linesofcode by -4 \the\linesofcode\ ``real'' lines. This compares favourably to a quality reference algorithm \cite{sedgewickcode} that has \myinput{sedgewickeslinesofcode.tex}

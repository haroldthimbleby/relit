...
\def\myinput#1{\IfFileExists{./#1}{\input #1\relax\unskip}{\textcolor{red}{\bfseries *** Warning: ``#1'' does not exist. Please run make~#1 to create the files that are needed! ***}}}
...
\new{There is no new notation to learn to get going, and when the full features are used, everything can be done with only two simple commands that are comments --- so they have no unintended side-effects on the meaning of the original document. No writing needs to be structured or re-structured (e.g., into an outline or set of sections as in conventional literate programming  and in systems like \cite{litprog,noweb}, Mathematica \cite{mathematica}, or org-mode  \cite{org-mode}, etc) to make use of it.}
...
\begin{enumerate}
\raggedright
\item Write the initial paper \emph{as usual}. (Of course, anticipating using \name{relit} one can do better than just write an ordinary paper.) \contrast{In conventional literate programming, one has to start with a special ``web'' file. No web files are in formats suitable for conventional publication.}

\item Edit the paper so program code snippets are preceded by either \name{relit} command \texttt{\%{}define}~$\mbox{\it name}_i$ or \texttt{\%{}generate}~$\mbox{\it name}_i$, using any convenient names. \contrast{In conventional literate programming, the corresponding definitions have to be done first (and are hard to change later).}

\item Optionally, somewhere that is ignored by \LaTeX\ (or whatever is used) --- in another file, or beyond the \verb|\end{document}| --- write a \texttt{\%{}generate $\mbox{\it filename}_j$ \ldots <name${}_i$> \ldots}. This will generate one or more files called $\mbox{\it filename}$ with the specified contents, which can include the named fragments of code along with any other housekeeping details as needed. \contrast{Conventional literate programming has no way to hide code that is distracting or needs no explanation.}

\item Compile and run or otherwise test the generated files.

\item Then edit the original paper to improve the code so it works better and/or the text of the paper more accurately describes it. \contrast{You cannot edit the original paper in literate programming; you have to edit the web file that generates the paper.}

\item Repeat until everything works as described.
...
\new{Note that as is good practice, the seed for the random number is stored in a variable: if any bug is found, a debugger can recover the value of the seed to reconstruct the same sequence of random numbers for a subsequent test run.}
...
Although normally ignored by \LaTeX\ and invisible to readers, the \name{relit} commands used to save the paragraph have been made clearly visible by defining {\tt \char'134relit} appropriately in \LaTeX, as can be seen highlighted on the two lines just before and just after the paragraph.  \new{This text can of course be easily defined by the author to suit their own purposes. The Appendix illustrates a real use for this feature.}
...
Here Feynman recalls the common habit we are not immune to in Computer Science when writing articles about algorithms (and when using other formal languages). As we refine and improve our articles, their connection with the original programs becomes tenuous. It has recently been argued that this bad science gives an effectively evolutionary advantage for the scientists who do it \cite{evolution}: cargo cult science is easier to publish --- for instance, which referees take the time to actually test programs discussed in papers, let alone work out exactly what the programs are so they can test them? Indeed, what authors even read all the papers they cite \cite{nocite}?

In turn this sloppiness creates a misleadingly ``productive'' culture which favours more of it: people and their promotion committees want longer CVs, not better papers. It becomes accepted practice, because everyone seems to benefit from taking the easiest route to publish more. 
...
\newpage
\section*{Appendix: All figure captions}
Many journals require a list of figure captions. The following list of figure captions was made automatically by using \name{relit} to generate a subscripted set of files, one for each figure caption, as described in section~\ref{subscript-section}. 

We simply redefined the \LaTeX\ macro \verb|\caption| (which places captions on figures) to work nicely outside of a figure. 

If we had been concerned, we could have used \name{relit}'s tags (see section \ref{invisible-code}) to distinguish captions that the reader of the paper can see and those that they cannot (for some reason) see --- for example, during editing a figure might have been moved beyond the \verb|\end{document}| so it was technically preserved in the \LaTeX\ source file, but was not part of the published document. Such things typically happen with co-authored papers, where one author wants to get rid of a figure (or other item) but dare not delete it completely and unnecessarily upsetting the other author --- the text will still be there, but it will not form part of the published paper. As it happens, we decided to keep all the figures!

\newcount\capN \capN=0

\def\caption#1{\global\advance \capN by 1\subsection*{Figure \the\capN\ caption}\noindent #1}

\myinput{caption-1.tex}
\myinput{caption-2.tex}
\myinput{caption-3.tex}
\myinput{caption-4.tex}
\myinput{caption-5.tex}

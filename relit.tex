...
\textbf{Dr. David Williams} MBChB, FRCA, Dip.\,DHM, PGCME is at Swansea
University, Wales.

David is a Consultant An\ae sthetist at Morriston Hospital, Swansea;
and Honorary Associate Professor and Lead for Simulation Training at Swansea
University Medical School.

He has research interests in: Human Factors, computing, and medical
device design.

In addition to over 100 peer reviewed publications, he is the Director
of four university spin-out companies, and has won several
international product design awards. He is an editor of \emph{An\ae
sthesia and Intensive Care Medicine}.
...
If we had been concerned, we could have used \name{relit}'s tags (see Section \ref{invisible-code}) to distinguish captions that the reader of the paper can see and those that they cannot (for some reason) see --- for example, during editing a figure might have been moved beyond the \verb|\end{document}| so it was technically preserved in the \LaTeX\ source file, but was not actually part of the published document. Such things typically happen with co-authored papers, where one author wants to get rid of a figure (or other item) but dare not delete it completely and unnecessarily upset the other author --- the text will still be there, but it will not form part of the published paper. As it happened, we decided to keep all the figures!

The list below shows how \name{relit} has automatically generated all of the caption files so that they are easy to include in a straight forward \LaTeX\ loop. However, since the full captions are of course printed with each figure in this paper, to save space we used a simple Unix filter to shorten each caption with an ellipsis.
\def\ellipsis{\unskip\ [$\cdot\cdot\cdot$]\ }

% simple recursive function to read as many caption files as there are
\newcount\captionNo \captionNo=1
\def\caption#1{Figure \the\captionNo\ caption: ``#1''\\}
\def\getcaption{\IfFileExists{shorter-caption-\the\captionNo.tex}{\myinput{shorter-caption-\the\captionNo.tex}\advance \captionNo by 1 \getcaption}{}}

\getcaption

\newpage
\section*{Appendix: Typesetting instructions}
Simply, just run \name{make}, and everything should happen automatically. However, for the purposes of this paper, explaining what happens step-by-step is more useful.

First, then, build the executable \name{relit} by compiling its C source file:

\begin{quote}\tt
cc relit.c -o relit
\end{quote}

Now use \name{relit} to generate the files from this paper by running: 

\begin{quote}\tt
./relit relit.tex
\end{quote}

For the present paper, some of these files are C and Java programs that need compiling and running to generate the examples used in the paper. \name{Make} has been set up to do all this correctly --- in fact, the makefile is also generated by \name{relit} --- so the details do not need to be shown here.

Now just treat everything in the standard \LaTeX\ way. Thus, the typeset document is generated as follows:

\begin{quote}
Run \texttt{pdflatex relit.tex} to get \texttt{relit.aux}, \texttt{relit.idx}, etc\\
Run \texttt{bibtex relit.aux} to get \texttt{relit.bbl}\\
Run \texttt{makeindex relit.idx} to get \texttt{relit.ind}
\end{quote}

This will generate the bibliography and the index (used in Figure \ref{fig:index}) for this paper. As usual, you will typically need to run all this least twice: to get cross references correct and to get the index page numbers correct --- you have to run \name{makeindex} twice, once to generate an index, then after running \name{pdflatex} again, to get the right page numbers after the index has been inserted into the document --- inserting the index itself into the document changes page numbers, so another run is needed to get the right page numbers.

\newpage
\section*{Useful, but not for publishing}

Below is a list of the generated files (i.e., files generated directly or indirectly by \name{relit} needed to typeset this paper correctly) in the order as they are used in the document. There are repetitions, because some are used more than once in this paper, for example the file lastwine.tex contains just one word, ``\myinput{lastwine.tex}\unskip,'' and is used several times in one paragraph (which was written on a single line in the \LaTeX\ source file) as well used in this very paragraph as the final file included. (If any files do not exist or cannot be read, the list below will have appropriate error messages in it.)\vskip 2ex

\begin{quote}
\listOfFiles
\end{quote}

%(In this published paper, depending on how the journal processed the \LaTeX\ and paginated the journal issue, the page numbers may be incorrect.)

\newpage
\section*{Figure captions for Elsevier}
% simple recursive function to read as many caption files as there are
\newcount\captionNo \captionNo=1
\def\caption#1{\vbox{\subsection*{Figure \the\captionNo\ caption} #1}}
\def\getcaption{\IfFileExists{caption-\the\captionNo.tex}{\myinput{caption-\the\captionNo.tex}\advance \captionNo by 1 \getcaption}{}}
...
\end{document}
% everything after \end{document} is unseen
%set-tag \unseen{}

%generate nameDemo.c /A/+1,/B/-1,none
A
<demo>
B
...
%generate randomised-euler.c ., /%end/-1, none
#define N 3 // for a graph with N vertices
<declare-walked>
<declare-initialise>
<random-declarations>
...
% clear the tag so we don't generate tagged files we aren't going to use
%set-tag none
